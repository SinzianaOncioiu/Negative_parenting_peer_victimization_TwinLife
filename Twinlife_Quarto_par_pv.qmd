---
title: "Does negative parenting behavior lead to later peer victimization?
...
knitr:
  opts_knit:
    root.dir: "C:/Users/Lenovo/Desktop/internships/research assistant position/twin life/Twin Life files/Data/SPSS_en"
---
...
-   Positive aspects:
    ...
    -   Emotional Warmth: par0100(t/u/s), par0101(t/u/s), par0102(t/u/s), par0103(t/u/s)

    -   Monitoring: par0109(t/u/s), par0110(t/u/s)
...
-   eca0106 - parental education (ISCED score: eca0106) - mother

-   inc0411 - net equivalent income; household income

-   inc0401 - 'normal' hh-income; household income
...
#sex, inc0411 - net equivalent income; household income, maternal mental health: dia0900, dia1000, dia1100, cohabiting: fam0100 'marital status', hpc - Number of people in HH,
...
### Restructuring the dataset containing mother-reported variables

Several parenting-related variables (e.g., harsh parenting, negative communication, psychological control, inconsistent parenting, externalizing, and internalizing behaviour) are originally stored in separate columns for each twin.
...
### **Creating Twin-Pair Variables by Swapping Values Between Twins in a Dataset with Both Row- and Column-Wise Twin Data**

This code processes a dataset where each twin pair is represented in separate rows but also has variables structured by twin (e.g., `harshpar_bymom_t1` for Twin 1). Using the family ID (`fid_num`), it identifies co-twins and swaps their values for specific parenting and behavioural variables. The process ensures that for each measure, values from Twin 1 are assigned to Twin 2 and vice versa, while maintaining the row-wise twin structure. Newly generated columns (`_t2` variables) store the swapped values to facilitate twin-comparison analyses.
...
### Add variable to select at random one twin per family
...
data.copy_int$victim_coh2_t1_scale <- as.numeric (scale(data.copy_int$victim_coh1_t1, center=TRUE, scale=TRUE))
data.copy_int$victim_coh2_t2_scale <- as.numeric (scale(data.copy_int$victim_coh1_t2, center=TRUE, scale=TRUE))

data.copy_int$ppl_hh <-as.numeric (data.copy_int$ppl_hh)
data.copy_int$cohabit <-as.factor (data.copy_int$cohabit)
data.copy_int$mom_MH <-as.factor (data.copy_int$mom_MH)
...
system.time (
  ...
  {# Define EXPOSURE
    ...
    sex_diffs_model  <- summary(gee(victim_coh1_t1_scale ~ X1 + sex_recod_f + X1:sex_recod_f +income+mom_MH+cohabit+ppl_hh+ ext_par_c1+int_par_c1, data=data.copyss, id=fid_num, corstr="exchangeable"))
    ...
    print("gaussian pheno reg")
    sex_diffs_model_males <- summary(gee(victim_coh1_t1_scale ~ X1 + sex_recod_f + X1:sex_recod_f +income+mom_MH+cohabit+ppl_hh+ ext_par_c1+int_par_c1, data=data.copyss, id=fid_num, corstr="exchangeable"))
    #sex_diffs_model_males <- summary(gee(Y1 ~ prag_yes1 + sex_recod1 + prag_yes1:sex_recod1 + str_yes1, data=data.copyssc, id=randomfamid, corstr="exchangeable"))
    #ADJUSTED
    # sex_diffs_model_males <- summary(gee(Y1 ~ DLD1_bin + sex_recod1 + DLD1_bin:sex_recod1+ lnvbcog_1+lpsbeht1, data=data.copyssc, id=randomfamid, corstr="exchangeable"))
    # Extract CV estimate (effect of cyber-victimisation versus no cyber-victimisation in males)
    ...
    data.copyss$int_mean <- NA;  data.copyss$int_mean <- rowMeans( data.copyss[,c("int_par_c1", "int_c1_t2")]) #family mean (Bb)
    data.copyss$int_diff1 <- NA; data.copyss$int_diff1 <- data.copyss$int_par_c1- data.copyss$int_mean #difference score for twin 1 (Bw)
 
    data.copyss$ext_mean <- NA;  data.copyss$ext_mean <- rowMeans(data.copyss[,c("ext_par_c1", "ext_c1_t2")]) #family mean (Bb)
    data.copyss$ext_diff1 <- NA; data.copyss$ext_diff1 <- data.copyss$ext_par_c1- data.copyss$ext_mean #difference score for twin 1 (Bw)
    ...
    sex_diffs_MZ_female <- summary(gee(victim_coh1_t1_scale ~ par_mean+ par_diff1 + sex_recod_f + par_diff1:sex_recod_f +int_mean+int_diff1+ ext_mean+ext_diff1,
                                       data=data.copy_mz, id=fid_num, corstr="exchangeable"))
    ...
    sex_diffs_MZ_male <- summary(gee(victim_coh1_t1_scale ~ par_mean+ par_diff1 + sex_recod1 + par_diff1:sex_recod1+int_mean+int_diff1+ ext_mean+ext_diff1,
                                     data=data.copy_mz, id=fid_num, corstr="exchangeable"))
...
system.time (
  ...
  {# Define EXPOSURE
    ...
    sex_diffs_model  <- summary(gee(victim_coh2_t1_scale ~ X1 + sex_recod_f + X1:sex_recod_f +income+mom_MH+cohabit+ppl_hh+ ext_self_c2 +int_self_c2 , data=data.copyss, id=fid_num, corstr="exchangeable"))
    ...
    print("gaussian pheno reg")
    sex_diffs_model_males <- summary(gee(victim_coh2_t1_scale ~ X1 + sex_recod_f + X1:sex_recod_f +income+mom_MH+cohabit+ppl_hh+  ext_self_c2 +int_self_c2, data=data.copyss, id=fid_num, corstr="exchangeable"))
    #sex_diffs_model_males <- summary(gee(Y1 ~ prag_yes1 + sex_recod1 + prag_yes1:sex_recod1 + str_yes1, data=data.copyssc, id=randomfamid, corstr="exchangeable"))
    #ADJUSTED
    # sex_diffs_model_males <- summary(gee(Y1 ~ DLD1_bin + sex_recod1 + DLD1_bin:sex_recod1+ lnvbcog_1+lpsbeht1, data=data.copyssc, id=randomfamid, corstr="exchangeable"))
    # Extract CV estimate (effect of cyber-victimisation versus no cyber-victimisation in males)
    ...
    data.copyss$int_mean <- NA;  data.copyss$int_mean <- rowMeans( data.copyss[,c("int_self_c2", "int_self_c2_t2")]) #family mean (Bb)
    data.copyss$int_diff1 <- NA; data.copyss$int_diff1 <- data.copyss$int_self_c2- data.copyss$int_mean #difference score for twin 1 (Bw)
 
    data.copyss$ext_mean <- NA;  data.copyss$ext_mean <- rowMeans(data.copyss[,c("ext_self_c2", "ext_self_c2_t2")]) #family mean (Bb)
    data.copyss$ext_diff1 <- NA; data.copyss$ext_diff1 <- data.copyss$ext_self_c2- data.copyss$ext_mean #difference score for twin 1 (Bw)
    ...
    sex_diffs_MZ_female <- summary(gee(victim_coh2_t1_scale ~ par_mean+ par_diff1 + sex_recod_f + par_diff1:sex_recod_f +int_mean+int_diff1+ ext_mean+ext_diff1,
                                       data=data.copy_mz, id=fid_num, corstr="exchangeable"))
    ...
    sex_diffs_MZ_male <- summary(gee(victim_coh2_t1_scale ~ par_mean+ par_diff1 + sex_recod1 + par_diff1:sex_recod1+int_mean+int_diff1+ ext_mean+ext_diff1,
                                     data=data.copy_mz, id=fid_num, corstr="exchangeable"))
